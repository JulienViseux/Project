...
DEFINITION:
CLE ETRANGERE: 
 elles permettent de gérer des relations entre plusieurs tables
 et garantissent la cohérence des données
CLE PRIMAIRE:
 elles permettent d'identifier une ligne de manière unique.
...
CREATE USER 'Théo'@'10.41.30.87' IDENTIFIED BY '1';
CREATE USER 'Julien'@'10.138.33.154' IDENTIFIED BY '2';
...
WHERE valeur_temp IS NOT NULL AND id_utilisateur = 1
GROUP BY id_piece LIMIT 1) as acqui_cap,
...
WHERE status_ampoule = 'on' AND id_utilisateur = 1
GROUP BY id_piece LIMIT 1) as nbr_ampoule,
...
SET @diff = ((SELECT AVG(valeur_temp) 
...
SELECT id_piece_thermostats, nom_thermostat, temp_cible, @diff,
(@diff - ((SELECT AVG(valeur_temp) 
FROM donnees_temperature as D 
